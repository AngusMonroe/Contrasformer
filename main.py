...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs):
    ...
    try:
        for split_number in range(10):
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    if optimizer.param_groups[0]['lr'] < params['min_lr']:
                        ...
                        if MODEL_NAME in ['ContrastGPS']:
                            contrast_graph = torch.cat(
                                [model.transformers[0].adj, model.transformers[1].adj], dim=0)
                            torch.save(contrast_graph, '{}.pkl'.format(log_dir + "/epoch_" + str(epoch)))
                        break
            ...
            _, test_acc, test_precision, test_recall, test_f1, test_roc_auc = evaluate_network_all_metric(model,
                                                                                                          device,
                                                                                                          test_loader,
                                                                                                          epoch)
...
def main():
    ...
    if MODEL_NAME in ['DiffPool', 'ContrastPool']:
        ...
        # calculate assignment dimension: pool_ratio * largest graph's maximum number of nodes  in the dataset
